...
//TODO: move everything here that isnt bolted to the floor into finetune.h
//TODO: why cant we just remove s here? this isnt called anywhere else for now
//TODO:  ^ is viable but we need to extern C. bindgen claims cpp but not reliable.
...
//a function that takes nothing and returns a std::vector<my_llama_layer>
std::vector<my_llama_layer> new_my_llama_layer() {
    std::vector<my_llama_layer> layers;
    return layers;
}
struct my_llama_model init_my_llama_model() {
    printf("in init_my_llama_model");
    //flush
    fflush(stdout);
    struct my_llama_model model;
    model.hparams = my_llama_hparams();
    //initialize the pointers
    model.tok_embeddings = NULL;
    model.norm = NULL;
    model.output = NULL;
    ...
    printf("init vec");
    fflush(stdout);
    model.layers = std::vector<my_llama_layer>{my_llama_layer()};
    printf("init vec complete");
    fflush(stdout);
    return model;
}
...
//a function that takes no arguments and constructs and initializes my_llama_lora with a vector for layers
struct my_llama_lora * new_my_llama_lora() {
    struct my_llama_lora * lora = new my_llama_lora();
    lora->layers.resize(32);
    return lora;
}
